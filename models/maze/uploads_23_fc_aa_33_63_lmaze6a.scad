/*
    ...
    Maze generator at http://burningsmell.org/maze.html
...
module polar(x,y, off=bolt_r) {
	rotate([0,0,arcw]*x) 
        translate([off,0,(y*bolt_h)/(rows+1)]) 
            children();
}
    ...
    translate([0,0,w2]/2) 
        rotate([-90,0,0]) intersection() {
            linear_extrude(w2, center=true) 
                hull() {
                    trapz(off=0);
                    translate([0,0.95*(l*bolt_h)/rows]) 
                        trapz(off=0);
                }
                    rotate([90,180,180]) 
                        translate([0,0,-5]) 
                            linear_extrude(10+((l*bolt_h)/rows)) 
                                trapz(off=0);
        }
}
...
module wedge(l, xtra=10) difference() {
	intersection() {
		owedge();
		rotate([0,0,270]) 
            pie(bolt_r+10, xtra, -xtra-arcw*l, 6);
	}

	rotate([180,0,5-xtra/2]) 
        translate([-.1+bolt_r-depth, 0, -7]) 
            rotate([0,0,30]) 
                cube([10,10,10]);
	rotate([0,0,-5+(arcw*l)+xtra/2])	
        translate([-.1+bolt_r-depth, 0, -3]) 
            rotate([0,0,30]) 
                cube([10,10,10]);
}
	...
	rotate([0,0,aoff]) union() {
		...
		for(y=[0:0]) 
            for(x=[0:sectors-1]) { 
                h = ylen(m,x,y) - 1; 
                if (h > 0) {
                    polar(x,3+rows-y-1,off=bolt_r-depth) 
                        beam(h+3);
                }
            }
		...
		for(y=[1:rows-1])	
            for(x=[0:sectors-1]) {
                h=ylen(m,x,y) - 1; 
                if (h > 0) {
                    polar(x,rows-y-1,off=bolt_r-depth) 
                        beam(h);
                }
            }  
		...
		for(y=[0:rows-1])
            for(x=[0:sectors-1])
                if(xlen(m[rows-y-1], x)) 
                    polar(x,y,off=0) 
                        wedge(xlen(m[rows-y-1],x)-1);
	}
